...
5. **Audit every row in `features[]` and `items[]`** — the whole plan, whatever a feature's Covered
   box says. An item row has no `category` and no Covered box, so never propose either in a `fix`.
