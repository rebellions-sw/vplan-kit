...
| `testcases[]` | **how** — UVM test class, virtual sequence, per-agent sequences, config, checks, and the rows it exercises: `feature_refs[]` and `item_refs[]`. An item no testcase names is a lint warning — that is how "every item is covered" is kept honest. `TC###` |
