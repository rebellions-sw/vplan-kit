...
| `features[]` | **what** must be verified — `category`, `name`, `description`, `priority`, `status`, `reviewed` (the **Verified** column), `notes`, and `related_refs[]`, a one-way link to features whose category is `command` (`REL_CAT` in the renderer). Its "Related to" half is rendered only on rows that are NOT `command` — a command row is what the others point at. Those chips show the command's NAME and are plain text; the verification chips (`Verified by`, `Link to`) show ids and open the peek drawer. `F##` |
