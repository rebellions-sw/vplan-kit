...
| `items[]` | **verification items** — what must hold for a feature to be true, one judgeable claim each, with its `oracle`, `judged_by` (SOM-VIP / IP-VIP / sva / test/seq / scoreboard / ref-model) and the `phase` it is due in (pre-Alpha / Alpha / Beta). `VI###` |
