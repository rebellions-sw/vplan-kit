...
| `testcases[]` | **how** — per-agent sequences, checks, TB hints, and the features it exercises, linked by chip and picker (`tcf`) exactly as the features and items link each other. **The verification items are derived, never stored** — `tcItems()` is the union of the items linked to those features, rendered read-only, and lint reads item coverage from the same function, so the two can never disagree. A chip opens the peek drawer on the row it names. Laid out like the Features / Verification Items tables (row + sub-rows, no expander). Columns run `ID · Type · Test · Phase · Definition · Implemented` — the same `phase` / `status` / `implemented` triple the items carry, so a testcase is judged due the same way a row is. No `owner` (the plan has one, in the top bar), no `seeds` (a regression setting), no `priority` (phase says when it is needed), no `coverage_refs` / `pass_criteria` / `config`, and no `dependencies` or `uvm.test_class` / `base_test` / `virtual_sequence` — coverage is tracked in its own tab, what passing means is the items' oracles, and class names and run order belong to the testbench, not to the plan; a plan that still carries them is stripped on load by `stripTestcase`. Refs: `feature_refs[]` and `item_refs[]`. An item whose features no testcase links is a lint warning when that item is `directed` — an `always-on` check needs no testcase of its own, which is what keeps a broad random test from being credited with half the plan. `TC###` |
