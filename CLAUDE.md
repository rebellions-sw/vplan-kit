...
| `testcases[]` | **how** — UVM test class, virtual sequence, per-agent sequences, config, checks, and the rows it exercises. Laid out like the Features / Verification Items tables (row + sub-rows, no expander). Columns run `ID · Type · Test · Phase · Definition · Implemented` — the same `phase` / `status` / `implemented` triple the items carry, so a testcase is judged due the same way a row is. No `owner` (the plan has one, in the top bar), no `seeds` (a regression setting, not a plan decision) and no `priority` (phase says when it is needed); a plan that still carries them is stripped on load by `stripTestcase`. Refs: `feature_refs[]` and `item_refs[]`. An item no testcase names is a lint warning when that item is `directed` — an `always-on` check needs no testcase of its own, which is what keeps a broad random test from being credited with half the plan. `TC###` |
